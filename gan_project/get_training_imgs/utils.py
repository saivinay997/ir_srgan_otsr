...
def imresize(img, scale, antialiasing=True): # antialiasing means adding grey pixels to the image along the edges
    # Now the scale should be the same for H and W
    # input: img: CHW RGB [0,1]
    # output: CHW RGB [0,1] w/o round

    in_C, in_H, in_W = img.size()
    _, out_H, out_W = in_C, math.ceil(in_H * scale), math.ceil(in_W * scale) # reducing the size of the image by scale factor
    kernel_width = 4
    kernel = 'cubic'
