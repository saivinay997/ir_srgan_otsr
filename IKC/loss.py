...
class GANLoss(nn.Module):
    ...
    def forward(self, input, target_is_real):
        ...
        if self.gan_type == "wgan-qc":
            loss = self.loss(input[0], target_is_real, input[1]) #pred_d, traget, HStar

        else: #"wgan-gp, gan, lsgan
...
class GradientPenaltyLoss(nn.Module):
    ...
    def forward(self, interp, interp_crit):
        ...
        grad_interp = torch.autograd.grad(outputs=interp_crit, inputs=interp,
                                          ...
                                          retain_graph=True, only_inputs=True)[0] #Computes and returns the sum of gradients of outputs with respect to the inputs.
